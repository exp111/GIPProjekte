...
void draw_boxes(Box boxes[])
{
	...
	{
		Box b = boxes[i];
		gip_draw_rectangle(b.x, b.y, b.x + box_size, b.y + box_size, blue);
	}
}
// update_boxes():
...
void handle_mouse_click(Box boxes[])
{
	...
	for (int i = 0; i < box_max; i++)
	{
		if (mouse_x >= boxes[i].x && mouse_x <= (boxes[i].x + box_size)) //X
		{
			if (mouse_y >= boxes[i].y && mouse_y <= (boxes[i].y + box_size)) //Y
			{
				boxes[i].y = 0;
				boxes[i].delta_y += 10;
			}
